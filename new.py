...
with dataset:
    ...
    st.line_chart(ten)
    st.line_chart(n_projects)
    st.line_chart(department)
    st.line_chart(status)    
    st.line_chart(salary)


target = np.array(data.drop(['status'],1))
features = np.array(data['status'])
...
with model_training:
    ...
    def lr_param_selector(tenure,rating,projects,salary,salary1,satisfaction,age,department):
        prediction = regr.predict([[tenure, rating, n_projects, salary,salary1,satisfaction,age,department]])
        return prediction
    ...
    if st.button("Predict"):
        result = lr_param_selector(tenure, rating, n_projects, salary, salary1,satisfaction,age,department) 
    st.success('The employee will stay or not?{}'.format(result))
